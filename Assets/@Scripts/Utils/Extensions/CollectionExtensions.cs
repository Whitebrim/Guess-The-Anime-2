...
public static class CollectionExtensions
{
    ...
    {
        return subset.Except(set).IsEmpty();
    }


    /// <summary>
    ///     Swaps a dictionary so that the keys become the values and the values become the keys.
    ///     If multiple keys in the original contain the same value, some information will be lost. To preserve it, use
    ///     <see cref="InvertWithList{T1, T2}(IReadOnlyDictionary{T1, T2})" />
    /// </summary>
    ...
    {
        foreach (KeyValuePair<T1, T2> matchingEntry in dictionary.Where(kvp => kvp.Value.Equals(value)).ToArray())
        {
    ...
    /// <summary>
    ///     Like <see cref="Enumerable.SequenceEqual{TSource}(IEnumerable{TSource}, IEnumerable{TSource})" />, but it accepts null parameters, and treats
    ///     them the same as an empty enumerable.
    /// </summary>
    ...
    public static bool AllElementsHaveTheSameProperty<TSource, TProperty>(this IEnumerable<TSource> sourceEnumerable,
    ...
    public static bool HasTheSameKeysAndValuesAs<TKey, TValue>(this IReadOnlyDictionary<TKey, TValue> dicA, IReadOnlyDictionary<TKey, TValue> dicB,
